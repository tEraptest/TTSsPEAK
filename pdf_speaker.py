...
def load_tts_model():
    ...
    try:
        ...
        tts_model = model
        tts_ready = True
        print(f"Модель Silero TTS ({MODEL_ID}, {SPEAKER}) загружена успешно.")
        return True
...
def generate_audio_chunk(text_to_speak, speed_multiplier=1.0):
    ...
    try:
        ...
        print(f"Генерация аудио для: '{cleaned_text[:60]}...' с скоростью {speed_multiplier:.1f}x")
...
def generate_full_mp3(pdf_document, output_path, start_page_idx, end_page_idx):
    print(f"Начало полной генерации MP3 для страниц {start_page_idx+1}-{end_page_idx+1}")
    ...
    try:
        for i, page_num in enumerate(range(start_page_idx, end_page_idx + 1)):
            ...
            for j, chunk in enumerate(text_chunks):
                audio_np = generate_audio_chunk(chunk, speed_multiplier=1.0)
        ...
        if 'list_file_path' in locals() and os.path.exists(list_file_path):
             ...
             except Exception as e_clean_list: print(f"Не удалось удалить файл списка {list_file_path}: {e_clean_list}")
...
class PdfReaderApp:
    ...
    def select_pdf(self):
        ...
        try:
            ...
            self.entry_start_page.delete(0, tk.END); self.entry_start_page.insert(0, "1")
            self.entry_end_page.delete(0, tk.END); self.entry_end_page.insert(0, str(self.total_pages))

            self.show_page(self.current_page_num)
            self.enable_controls()

        except Exception as e:
            self.pdf_document = None; self.pdf_path = ""; self.total_pages = 0
    ...
    def clear_display(self):
        self.pdf_image_label.config(image=''); self.pdf_image_label.image = None
        self.text_display.config(state=tk.NORMAL); self.text_display.delete('1.0', tk.END)
        self.text_display.config(state=tk.DISABLED)
        self.lbl_page_display.config(text="Страница: - / -")

    def show_page(self, page_index):
        if not self.pdf_document or not (0 <= page_index < self.total_pages): return
        ...
        try:
            ...
            self.pdf_image_label.config(image=img_tk); self.pdf_image_label.image = img_tk
        except Exception as e:
        ...
        try:
            ...
            self.text_display.config(state=tk.NORMAL); self.text_display.delete('1.0', tk.END)
            self.text_display.insert('1.0', text); self.text_display.config(state=tk.DISABLED)
        except Exception as e:
            print(f"Ошибка извлечения текста со страницы {page_index + 1}: {e}")
            self.text_display.config(state=tk.NORMAL); self.text_display.delete('1.0', tk.END)
            self.text_display.insert('1.0', f"Ошибка текста стр. {page_index + 1}"); self.text_display.config(state=tk.DISABLED)
    ...
    def prev_page(self):
        if self.pdf_document and self.current_page_num > 0: self.show_page(self.current_page_num - 1)
    def next_page(self):
        if self.pdf_document and self.current_page_num < self.total_pages - 1: self.show_page(self.current_page_num + 1)

    def play_range(self):
        if not self.pdf_document: messagebox.showwarning("Нет PDF", "Сначала загрузите PDF."); return
        if not tts_ready: messagebox.showerror("TTS не готов", "Модель озвучивания не готова."); return
        if self.playing_thread and self.playing_thread.is_alive(): messagebox.showinfo("Занято", "Воспроизведение уже идет."); return
        ...
        thread_args = (start_page, end_page)
    ...
    def _play_audio_thread(self, start_page_idx, end_page_idx):
        print(f"Запуск озвучки со стр {start_page_idx + 1} по {end_page_idx + 1}")
        ...
        try:
            for page_num in range(start_page_idx, end_page_idx + 1):
                if self.stop_playback_flag.is_set(): print("Остановка (начало цикла стр)."); page_processed_successfully = False; break
                if not self.pdf_document: print("PDF закрыт (начало цикла стр)."); page_processed_successfully = False; break

                self.root.after(0, lambda p=page_num: self.show_page(p))
                self.root.after(0, self.update_status, f"Обработка страницы {page_num + 1}...")
                time.sleep(0.1)
                if self.stop_playback_flag.is_set(): print("Остановка (после показа стр)."); page_processed_successfully = False; break

                try:
                    page = self.pdf_document.load_page(page_num)
                    full_page_text = page.get_text("text", sort=True)
                except Exception as e:
                    print(f"Ошибка чтения текста стр {page_num + 1}: {e}"); time.sleep(1); continue

                if not full_page_text or full_page_text.isspace():
                    self.root.after(0, self.update_status, f"Стр. {page_num + 1}: нет текста."); time.sleep(0.5); continue

                text_chunks = split_text_into_chunks(full_page_text)

                for i, chunk in enumerate(text_chunks):
                    if self.stop_playback_flag.is_set(): print("Остановка (начало цикла фрагментов)."); page_processed_successfully = False; break
                    if not self.pdf_document: print("PDF закрыт (начало цикла фрагментов)."); page_processed_successfully = False; break
                    ...
                    audio_np = generate_audio_chunk(chunk, speed_multiplier=self.speech_speed)
                    ...
                    if audio_np is not None and audio_np.size > 0:
                        ...
                        if not play_success:
                         ...
                         page_processed_successfully = False; break

                    if self.stop_playback_flag.is_set(): print("Остановка (после фрагмента)."); page_processed_successfully = False; break

                if not page_processed_successfully: break
    ...
    def save_full_audio_to_mp3(self):
        if not self.pdf_document: messagebox.showwarning("Нет PDF", "Загрузите PDF."); return
        if not tts_ready: messagebox.showerror("TTS не готов", "Модель не загружена."); return
        ...
        try:
            ...
            messagebox.showerror("Ошибка диапазона", f"Некорректный диапазон страниц (от 1 до {self.total_pages})."); return
    ...
    def _run_full_save_mp3(self, output_path, start_page, end_page):
         try:
             ...
             success = generate_full_mp3(doc_to_use, output_path, start_page, end_page)
            ...
            with open(list_file_path, 'w', encoding='utf-8') as f:
                for wav_file in files_to_concat:
                    if os.path.exists(wav_file):
                        ...
                        print(f"Предупреждение: временный файл {wav_file} не найден, пропускаем.")

            # Проверяем, есть ли что объединять
            if not os.path.exists(list_file_path) or os.path.getsize(list_file_path) == 0:
                 print("Файл списка для ffmpeg пуст или не создан. Нечего сохранять.")
                 self.root.after(0, messagebox.showwarning, "Нечего сохранять", "Не удалось найти временные аудиофайлы для объединения.")
                 self.root.after(0, self.update_status, "Нет данных для сохранения.")
                 return # Выходим из функции потока
        ...
        for f_path in files_to_remove:
            try:
                if os.path.exists(f_path):
                    ...
                    if dir_name: dirs_to_check.add(dir_name)
                    os.remove(f_path)
                    cleaned_count += 1
                else:
    ...
    def highlight_text(self, full_text, chunk):
        try:
            if not self.text_display.winfo_exists(): return
